...
const DOCUMENT_TYPE = 'sewa_tempat';

export default function SewaTempat() {
  ...
  const handleDownload = () => {
    if (fileUrl) {
      ...
      link.href = "/dokumen/syarat-peminjaman-gedung.pdf"; // Path file di public
      link.download = "Syarat_Peminjaman_Gedung.pdf";
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 group flex flex-col overflow-hidden h-full">
      {/* Header Visual */}
      <div className="p-6 bg-gradient-to-br from-blue-500/5 to-cyan-500/10 border-b border-blue-50/50 relative overflow-hidden">
        <div className="absolute -right-4 -top-4 w-28 h-24 bg-blue-500/20 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700"></div>
        
        <div className="w-12 h-12 bg-white rounded-2xl shadow-sm border border-blue-100 flex items-center justify-center text-blue-600 mb-4 relative z-10 group-hover:-translate-y-1 transition-transform duration-300">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2-2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
          </svg>
        </div>
        
        <h3 className="text-lg md:text-xl font-bold text-gray-800 relative z-10">Sewa Tempat & Venue</h3>
        <p className="text-xs md:text-sm text-gray-500 mt-1 relative z-10">MoU peminjaman gedung, kuitansi lunas, dan surat izin kepolisian.</p>
      </div>
      ...
      <div className="p-6 flex-1 flex flex-col justify-between gap-6">
        ...
        <div>
          {isLoading ? (
            ...
            <div className="flex items-center gap-3 p-3 bg-blue-50/50 border border-blue-100 rounded-xl animate-pulse">
              <div className="w-5 h-5 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
              <p className="text-xs font-medium text-blue-700">Mengunggah dokumen...</p>
            </div>
          ) : fileName ? (
            ...
            <div className="flex items-center gap-3 p-3 bg-gray-50 border border-gray-200 rounded-xl border-dashed">
              ...
              <p className="text-xs font-medium text-gray-500 italic">Belum ada dokumen yang diunggah.</p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-auto">
          <input type="file" className="hidden" ref={fileInputRef} onChange={handleFileChange} accept=".pdf,.doc,.docx,.jpg,.png" />
          <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white py-2.5 px-3 rounded-xl text-xs md:text-sm font-bold transition-all shadow-sm flex items-center justify-center gap-2 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed">
          ...
          <button onClick={handleDownload} className="flex-1 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 py-2.5 px-3 rounded-xl text-xs md:text-sm font-bold transition-all flex items-center justify-center gap-2 active:scale-95">
            ...
            {fileName ? "Unduh Berkas" : "Syarat Gedung"}
          </button>
        </div>
      </div>
    </div>
